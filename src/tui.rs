...
impl TuiApp {
    ...
    pub fn run(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.event_storage
            .info("Server", &self.event_storage.get_server_info());
                ...
                match self.active_tab {
                    AppTab::Events => {
                        if modifiers.contains(KeyModifiers::CONTROL) {
                            ...
                            let scroll_amount = if modifiers.contains(KeyModifiers::SHIFT) {
                                10
                            } else {
                                1
                            };
            ...
            KeyCode::Char('C') => {
                match self.active_tab {
                    ...
                    _ => {
                        ...
                        self.event_storage
                            .info("TUI", "Cleared all events and logs");
                        self.event_storage.clear_events();
                        self.event_storage.clear_logs();
                        self.selected_index = 0;
                        self.selected_log_index = 0;
    ...
    fn render_events_tab(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let events_state = &ListState::default().with_selected(if events.is_empty() {
            None
        } else {
        ...
        if let Some(event) = events.get(self.selected_index) {
            ...
            let metadata = Table::new(vec![Row::new(vec![
            ...
            let details_widget = Paragraph::new(visible_content)
                .block(
                    Block::default()
                        .title(format!(
                            "Content ({} format){}",
                            &event.content_type, scroll_indicator
                        ))
                        .borders(Borders::ALL),
                )
                .wrap(Wrap { trim: true });
    ...
    fn render_logs_tab(&mut self, frame: &mut Frame, area: Rect) {
        ...
        for log_entry in app_logs.iter().rev() {
            ...
            let content = Line::from(vec![
                ...
                Span::styled(
                    format!("[{}] [{}] ", level_str, log_entry.source),
