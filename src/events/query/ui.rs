...
pub fn render_query_event(entry: &EventEntry, cx: &mut Context<crate::app::MyApp>) -> Div {
...
fn render_query_metrics(content: &Value) -> Div {
    ...
    let connection = content.get("connection_name").and_then(|c| c.as_str()).unwrap_or("default");
    ...
    styled_card()
        ...
        .child(
            div()
                ...
                .flex_row()
                .gap_6()
                .child(render_metric("⏱️".to_string(), "Execution Time".to_string(), time_display, time_color))
                .child(render_metric("🔗".to_string(), "Connection".to_string(), connection.to_string(), gpui::rgb(0x6b7280)))
    ...
    for i in 0..5 {
        container = container.child(
            div()
                ...
                .h_1()
                .rounded_sm()
                .bg(if i < filled { color } else { gpui::rgb(0xe5e7eb) })
        );
    }
...
fn highlight_sql_keywords(sql: &str) -> String {
    ...
    let keywords = [
        ...
        "DEFAULT", "AUTO_INCREMENT", "UNIQUE", "CONSTRAINT", "ON", "AS",
        "GROUP", "BY", "ORDER", "HAVING", "LIMIT", "OFFSET", "DISTINCT",
        "AND", "OR", "IN", "LIKE", "BETWEEN", "IS", "EXISTS", "UNION",
        "VARCHAR", "INT", "BIGINT", "DECIMAL", "DATE", "DATETIME", "TEXT"
    ];
