...
pub fn process(payload: &Value) -> Result<EventEntry> {
    ...
    if let Some(content) = payload.get("content") {
        ...
        if label == "Cache" {
            process_cache_event(&mut entry, content)?;
        }
        // Handle HTTP requests and responses
        else if label == "Http" {
            process_http_event(&mut entry, content)?;
        }
        // Default handling for any other table type
        else if let Some(values) = content.get("values") {
            process_generic_table(&mut entry, label, values)?;
        }
...
fn process_cache_event(entry: &mut EventEntry, content: &Value) -> Result<()> {
...
fn process_http_event(entry: &mut EventEntry, content: &Value) -> Result<()> {
    if let Some(values) = content.get("values") {
        ...
        if is_request {
            process_http_request(&mut markdown, values, url, method.unwrap_or(""))?;
            entry.label = "HTTP: Request".to_string();
            entry.description = url.to_string();
        } else if is_response {
            process_http_response(&mut markdown, values, url, status.unwrap_or(0))?;
...
fn process_generic_table(entry: &mut EventEntry, label: &str, values: &Value) -> Result<()> {
